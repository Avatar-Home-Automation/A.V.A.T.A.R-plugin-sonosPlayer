...
async function ruleExists(searchValue) {
    ...
    return Object.values(mappingRules).some(valuesArray =>
        valuesArray.some(value => value.toLowerCase() === lowerSearch)
    );
}
...
document.getElementById("save-mapping").addEventListener("click", async (event) => {
    ...
    if (!document.getElementById("L-auto").toggled) {
        ...
        for (let i = 0; i < menuOn.length; i++) {
            if (menuOn[i].value.toLowerCase() === document.getElementById("new-mapping").value.toLowerCase()) {
                notification(await Lget("mapping.existMapping", document.getElementById("new-mapping").value), true);    
                return; 
            }
        }
    ...
    document.getElementById("save-mapping").disabled = true;
...
async function setMappingList(mappingList) {

    mappingList = Object.keys(mappingList);
...
window.electronAPI.onInitMapping(async (_event, music, mapping) => {
    mappingRules = mapping || {};
    await setTargets(music);
    await setMappingList(mapping);
    document.getElementById("youSay").value = music;
})