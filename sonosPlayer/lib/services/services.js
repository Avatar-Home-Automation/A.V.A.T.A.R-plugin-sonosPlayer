...
export class SonosAPI {
  ...
  constructor(mappingTTS, mappingRules) {
    if (!SonosAPI.instance) {
      this.SonosAPI = new Map();
      SonosAPI.instance = this;
    }
    this.mappingTTS = mappingTTS;
    this.mappingRules = mappingRules;
    return SonosAPI.instance;  
  }
  ...
  matchTerm(language, items, term) {
    ...
    if (match.length > 0) {
      info ('Compréhension au plus proche: ' + match[0]);
    } 
