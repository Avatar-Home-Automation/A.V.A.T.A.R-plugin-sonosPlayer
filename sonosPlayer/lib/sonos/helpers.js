...
Helpers.GenerateMetadata = function (uri, title = '', region = '3079') {
  var parts = uri.split(':')
  if (!((parts.length === 2 && (parts[0] === 'radio' || parts[0] === 'x-sonosapi-stream' || parts[0] === 'x-rincon-cpcontainer' || parts[0] === 'deezer_track_x-sonos-http')) || (parts.length >= 3 && parts[0] === 'spotify'))) {
  ...
  } else if (uri.startsWith('x-rincon-cpcontainer:1004204calbum') || uri.startsWith('x-rincon-cpcontainer:1004006calbum')) { // Deezer Albums or albums from Artists
    const itemId = parts[1].split('?')[0];
    return {
      uri: uri,
      metadata: meta.replace('##SPOTIFYURI##', itemId)
        .replace('##RESOURCETITLE##', title)
        .replace('##SPOTIFYTYPE##', 'object.container.album.musicAlbum')
        .replace('##PARENTID##', '') //'parentID="100d2464user-albums-4203955802" ')
        .replace(`SA_RINCON${region}_X_#Svc${region}-0-Token`, 'SA_RINCON519_X_#Svc519-0-Token')
    }
  } else if (uri.startsWith('x-rincon-cpcontainer:10060a6cplaylist_spotify')) { // Deezer playlist
    const itemId = parts[1].split('?')[0];
    return {
      uri: uri,
      metadata: meta.replace('##SPOTIFYURI##', itemId)
        .replace('##RESOURCETITLE##', title)
        .replace('##SPOTIFYTYPE##', 'object.container.playlistContainer')
        .replace('##PARENTID##', '') //'parentID="10fe2664playlists" ')
        .replace(`SA_RINCON${region}_X_#Svc${region}-0-Token`, 'SA_RINCON519_X_#Svc519-0-Token')
    }
  } else if (uri.startsWith('deezer_track_x-sonos-http:tr')) { // Deezer track
    uri = uri.replace('deezer_track_', '');
    const itemId = parts[1].split('.')[0];
    return {
      uri: uri,
      metadata: meta.replace('##SPOTIFYURI##', "10032020"+itemId)
        .replace('##RESOURCETITLE##', title)
        .replace('##SPOTIFYTYPE##', 'object.item.audioItem.musicTrack')
        .replace('##PARENTID##', '') //'parentID="10fe2664playlists" ')
        .replace(`SA_RINCON${region}_X_#Svc${region}-0-Token`, 'SA_RINCON519_X_#Svc519-0-Token')
