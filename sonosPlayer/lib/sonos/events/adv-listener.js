...
const parseXml = Helpers.ParseXml
import { EventParser } from './eventParser.js'
const ParseAndEmitEvents = EventParser.ParseAndEmitEvents

const listenAddress = process.env.SONOS_LISTENER || await getIpAddress()
...
class SonosListener extends EventEmitter {
    ...
    this._requestHandler = function (req, resp) {
      ...
      req
        ...
        .on('error', (err) => { // If we don't listen to this it will crash https://nodejs.org/en/docs/guides/anatomy-of-an-http-transaction/
          console.log('Error on http request %j', err)
        })
    }
  ...
  async subscribeTo (device) {
    return new Promise((resolve, reject) => {
      ...
      if (device._isSubscribed) {
        console.log('Already subscribed to this device')
        resolve(true)
        return
      }
  ...
  _handleMessage (req, resp, body) {
    ...
    if (method.toUpperCase() !== 'NOTIFY' || url.toLowerCase() !== '/notify') {
      console.log('Unknown request %s %s %j %s', method, url, headers, body)
      resp.statuscode = 404
      ...
      if (!deviceSubscription) {
        console.log('Request received for unknown subscription %s', headers.sid)
        ...
        parseXml(body)
          ...
          .catch(err => {
            console.log('Error parsing the body %j', err)
            resp.statuscode = 500
 ...
 */
  ...
  _startTimer () {
    ...
    this._renewTimer = setInterval(async () => {
      try {
        ...
        console.log('Timer error renewing subscriptions %j', err)
        if (err.cause && err.cause.code === 'EHOSTUNREACH') {
          console.log('Continue renewing subscription')
        } else {
          console.log('Stop renewing subscription')
  ...
  async cancelSubscription (sid) {
    return request({
      ...
      console.log('Cancelled subscription %s for %s', sid, this.device.host)
      delete this.subscriptions[sid]
    })
  }
